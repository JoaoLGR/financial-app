...
begin
  ...
  insert into public.transactions (workspace_id, type, description, amount, transaction_date, competence_date, status, account_id, transfer_group_id, transfer_direction)
  values (p_workspace_id, 'TRANSFER', coalesce(nullif(trim(p_description), ''), 'Transferência'), p_amount, p_transaction_date, p_transaction_date, 'PAID', p_source_account_id, transfer_id, 'OUT');
  insert into public.transactions (workspace_id, type, description, amount, transaction_date, competence_date, status, account_id, transfer_group_id, transfer_direction)
  values (p_workspace_id, 'TRANSFER', coalesce(nullif(trim(p_description), ''), 'Transferência'), p_amount, p_transaction_date, p_transaction_date, 'RECEIVED', p_destination_account_id, transfer_id, 'IN');
  return transfer_id;
end;
$$;
