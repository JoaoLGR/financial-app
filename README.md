...
Para convites por e-mail, configure o Site URL do Supabase para `https://financial-app-blond.vercel.app` e adicione `https://financial-app-blond.vercel.app/auth/callback` à lista de Redirect URLs. Em desenvolvimento, mantenha também `http://localhost:3000` e `http://localhost:3000/auth/callback`. O app finaliza a sessão em `/auth/callback`, encaminha o convidado para `/auth/set-password` e só então permite chegar ao dashboard.
